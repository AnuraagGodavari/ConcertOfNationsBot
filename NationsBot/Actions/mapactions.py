...
async def worldmap(client, ctx): 
    ...
    menu = MenuEmbed(
        ...
        "Territories are displayed by their IDs, but can be referenced by their names, too.", 
        ctx.author.id,
...
# Territory Util Functions

def territoriesMenu(client, ctx, roleid, shop = False):
    """
    ...
    if (shop and buildingInCart):
        ...
        menu.buttons = [
            CommandButton(ctx, client, f"{world[terr].name}", 1, territory, [world[terr].id])
            for terr in menu_territories
        ]

    return menu
    
def selectTerritory(ctx, terrID):
...
async def getTerritories(client, ctx, roleid = None):
    ...
    menu = territoriesMenu(client, ctx, roleid)
...
async def selectTerritoryMenu(client, ctx, roleid = None):
    """ 
    ...
    menu = territoriesMenu(ctx, roleid, shop = True)
...
async def territory(client, ctx, terrID):
